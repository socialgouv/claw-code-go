...
| Providers — anthropic + openai | yes | yes | COMPLETE | `pkg/api/providers/{anthropic,openai}`. OpenAI covers both `/v1/chat/completions` and `/v1/responses` (`internal/api/providers/openai/responses.go`). |
